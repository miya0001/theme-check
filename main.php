...
/**
 * Prints a form that allows the user to select a theme to sniff.
 */
function wptrt_tc_print_sniff_form() {
	?>
	<form action="themes.php?page=themecheck" method="post">
		<select name="theme-to-sniff">
			<?php
			foreach( tc_get_themes() as $name => $location ) {
				printf( '<option value="%s" %s>%s</option>',
					$location['Stylesheet'],
					selected( $location['Stylesheet'] === $_POST['theme-to-sniff'] || basename( STYLESHEETPATH ) === $location['Stylesheet'], true, false ),
					$name
				);
			}
			?>
		</select>
		<?php submit_button( esc_html__( 'Sniff it!', 'theme-check' ), 'secondary', 'sniff-it', false ); ?>
	</form>
	<?php
}
...
/**
 * Sniff a theme:
 * - Load CodeSniffer CLI.
 * - Set WordPress Theme coding standard.
 * - Run sniff and print results.
 *
 * @param string $theme Theme folder to sniff.
 */
function wptrt_tc_do_sniff( $theme ) {
	// Path to CodeSniffer CLI file.
	include_once 'vendor/phpcs/CodeSniffer/CLI.php';

	// Path to WordPress Theme coding standard.
	PHP_CodeSniffer::setConfigData( 'installed_paths', dirname(__FILE__) . '/vendor/wptrt-cs', true );
